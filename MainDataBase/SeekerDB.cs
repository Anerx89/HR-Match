...
public class SeekerDB
{
    ...
    public List<Job> GetJobNameAndID(List<int> jobID)
    {
        ...
        {
            var connection = new MySqlConnection(SeekerDB.sqlString);
            jobNames.Add(connection.QuerySingle<Job>($"SELECT job_title AS Job_title, job_id AS Job_id, job_location AS Job_location, job_description AS Job_description FROM job WHERE job_id={jobid};"));
        }
        return jobNames;
    }
    ...
    public List<Job> GetSeekerApplyHistory(int seekerID)
    {
        ...
        foreach (var id in seekerApply)
        {
            ...
            seekerJobList.Add(connection.QuerySingle<Job>($"SELECT job_title AS Job_title, job_id AS Job_id, job_location AS Job_location, job_description AS Job_description FROM job WHERE job_id={id};"));
        }
